# Modified by Tida, added Timeline Name Filter
# keep Filter empty for "All"
...
class Project():
    ...
    def all_timelines_in_current_project(self,qFilter):
        for i in range(1, self.project_manager.GetCurrentProject().GetTimelineCount()+1):
            if qFilter in (self.project_manager.GetCurrentProject().GetTimelineByIndex(i).GetName()):
...
timeline_level_group = ui.HGroup({"Spacing": 5, "Weight": 0},[
    ui.VGroup({"Weight": 1},[
        ...
        ui.VGap(),
        ui.Label({"Text": "Filter: ", "Alignment": {"AlignRight": True, "AlignVCenter": True}}),
    ]),
    ui.VGroup({"Spacing": 5, "Weight": 1},[
        ...
        ui.LineEdit({"ID": qFilter, "PlaceholderText": "__", "Editable": True}),
    ])
])
...
dlg = disp.AddWindow({ 
                        "WindowTitle": "Batch Exporter FilterMod", 
                        "ID": "MyWin", 
                        "Geometry": [ 
...
def _show_timeline_list(ev):
    ...
    if checkstate(AllTimeline):
        ...
        timelines = list(PROJECT.all_timelines_in_current_project(itm[qFilter].Text))
        toplevelitems = []
        for i in timelines:
...
def export_in_project(projectName):
    ...
    if checkstate(ExportProj):
        ...
        if checkstate(RenderTimeline):
            # render
            if checkstate(AllTimeline):
                timelines = p.all_timelines_in_current_project(itm[qFilter].Text)
